...
--3. Countries with the highest infection count compared to the population --> (Result used in TABLEAU for visualization)
select location, Population, Max(total_cases) as HighestInfectionCount, max(round((total_cases/Population)*100,1)) as PercentPopulationInfected
from [dbo].[CovidDeaths$]
where continent is not null
group by location,population
order by PercentPopulationInfected Desc

--3.1 Date wise highest infection count compared to the population --> (Result used in TABLEAU for visualization)
select location, Population,date, Max(total_cases) as HighestInfectionCount, max(round((total_cases/Population)*100,1)) as PercentPopulationInfected
from [dbo].[CovidDeaths$]
where continent is not null
...
--5. Group by continent with the highest death count per Population  --> (Result used in TABLEAU for visualization)
...
--6. Global Data (Total Cases and Total Deaths) --> (Result used in TABLEAU for visualization)
